...
function uploader(req, res) {
  ...
  if (req.body) {
    ...
    // replacing Windows file path separator '\' with the Linux based one '/'
    var targetFilename = req.body.file.path.replace(/\\/g, '/').split('/').slice(-1)[0];
    var uniqueId = req.app || req.user._id;
    var targetKey = UPLOAD_DIR + uniqueId + '/' + targetFilename;
