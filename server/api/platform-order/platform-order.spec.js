...
var token = '',
  userId = '',
  platformOrderId1 = mongoose.Types.ObjectId();
...
describe('/api/platform-orders', function() {
    ...
    User.find({}).remove(function() {
      User.create({
        ...
        MongoClient.connect(config.mongo.uri, function(err, db) {
          ...
          var platfCol = db.collection('platformorders');


          // Insert some documents
          platfCol.insert(dummyPlatformOrders, done);
        });
      });
    });
  });
  ...
  describe('GET /api/platform-orders', function() {
  ...
  describe('GET /api/platform-orders/:id', function() {

    it('should respond with JSON array', function(done) {
      request(app)
        .get('/api/platform-orders/' + platformOrderId1)
        .set('Authorization', getBearer())
        .expect(200)
        .expect('Content-Type', /json/)
        .end(function(err, res) {
  ...
  describe('POST /api/platform-orders/fastspring', function() {
    ...
    it('should respond with JSON object', function(done) {
      ...
      request(app)
        .post('/api/platform-orders/fastspring')
        .send(data)
        .expect(200)
        .expect('Content-Type', /json/)
        .end(function(err, res) {
